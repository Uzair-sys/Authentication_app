...
router.post("/email/signup", async (req, res) => {
  const { email, password ,discount_status} = req.body;
    ...
    User.findOne({ where: { email: email } }).then(async user => {
      if (user) {
        ...
        User.create({
          ...
          .catch(err => res.status(500).json({ errors: [{ msg: err.message,type:1 }] }));
      }
    });
  } catch (err) {
    res.status(400).json({ errors: [{ msg: err.message,type:2 }] });
  }
});
